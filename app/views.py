...
def index(request):
    ...
    # Handle new yfinance MultiIndex - flatten if needed
    # New yfinance returns data with MultiIndex columns by ticker then price type
    ...
    fig_left = go.Figure()
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data.get('Close_AAPL', []), name="AAPL")
            )
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data.get('Close_AMZN', []), name="AMZN")
            )
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data.get('Close_QCOM', []), name="QCOM")
            )
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data.get('Close_META', []), name="META")
            )
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data.get('Close_NVDA', []), name="NVDA")
            )
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data.get('Close_JPM', []), name="JPM")
            )
    ...
    # Download recent stocks data with error handling
    recent_stocks = []
    try:
        tickers_list = ['AAPL', 'AMZN', 'GOOGL', 'UBER', 'TSLA', 'NFLX']
        dfs = []
        
        for ticker in tickers_list:
            try:
                df_ticker = yf.download(tickers=ticker, period='1d', interval='1d', progress=False)
                if df_ticker.empty:
                    continue
                    
                # Handle MultiIndex columns from yfinance 0.2.66+
        ...
        if dfs:
            ...
            # Handle DataFrame columns after concatenation
            if isinstance(df.columns, pd.MultiIndex):
                df.columns = df.columns.get_level_values(0)
            
            # Select only the columns we need from available columns
            available_cols = df.columns.tolist()
            cols_to_keep = ['Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Volume']
            cols_to_keep = [col for col in cols_to_keep if col in available_cols]
            df = df[cols_to_keep]
            
            # Rename Close to Adj_Close to match template expectations
        ...
        print(f"Error processing recent stocks: {e}")
        recent_stocks = []
...
def ticker(request):
    # ================================================= Load Ticker Table ================================================
    ticker_df = pd.read_csv('app/Data/new_tickers.csv', index_col=0) 
...
def predict(request, ticker_value, number_of_days):
    # First, validate the number_of_days parameter
    try:
    ...
    # Handle new yfinance MultiIndex structure (columns have Price/Ticker levels)
    # Flatten column names if they are MultiIndex
    if isinstance(df_ml.columns, pd.MultiIndex):
        # New yfinance returns MultiIndex: e.g., ('Close', 'AAPL'), ('High', 'AAPL')...
        # Get the price type (Close, High, Low, Open, Volume)
        df_ml.columns = df_ml.columns.get_level_values(0)
    
    # Fetching ticker values from Yahoo Finance API
    # Use 'Close' instead of 'Adj Close' (new yfinance doesn't have 'Adj Close')
    close_column = 'Close' if 'Close' in df_ml.columns else 'Adj Close'
    if close_column not in df_ml.columns:
        return render(request, 'API_Down.html')
    
    df_ml = df_ml[[close_column]]
    forecast_out = int(number_of_days)
    df_ml['Prediction'] = df_ml[[close_column]].shift(-forecast_out)
